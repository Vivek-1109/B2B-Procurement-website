...
import { Plus, Pencil, Trash2, X, Search, Handshake } from 'lucide-react';
    ...
    <AdminLayout title="Clients" subtitle="Manage clients and partners displayed on the homepage marquee">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(client => (
          <div
            key={client.id}
            className="bg-white rounded-sm border border-[#E5E7EB] overflow-hidden group hover:shadow-md hover:border-[#1557B0]/30 transition-all flex flex-col justify-between"
          >
            {/* Image / Logo preview */}
            <div className="h-36 bg-[#F4F8FC] overflow-hidden relative flex items-center justify-center border-b border-[#E5E7EB]">
              {client.logoUrl ? (
                <img
                  src={client.logoUrl}
                  alt={client.name}
                ...
                <div className="text-center p-4">
                  <span className="w-10 h-10 rounded-md bg-[#1557B0]/10 text-[#1557B0] flex items-center justify-center font-serif text-lg font-extrabold mx-auto mb-2">
                    {client.name ? client.name.charAt(0).toUpperCase() : 'C'}
                  </span>
                  <span className="text-xs font-semibold text-[#6B7280] tracking-wider uppercase">{client.name}</span>
                </div>
              )}
              
              {/* Badges */}
              <div className="absolute top-2 left-2 flex flex-col gap-1">
                <span className="text-[10px] px-2 py-0.5 bg-[#1557B0] text-white rounded-sm font-medium">
                  Order: {client.order}
                </span>
            ...
            {/* Content */}
            <div className="p-4">
              <h3 className="font-semibold text-[#1F2937] text-sm mb-3 leading-snug line-clamp-1">
                {client.name}
              </h3>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-sm shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-[#E5E7EB] flex items-center justify-between sticky top-0 bg-white z-10">
              <h2 className="font-bold text-[#1F2937]">{editing ? 'Edit Client' : 'Add New Client'}</h2>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2.5 py-1">
                <input
                  ...
                  className="w-4 h-4 text-[#1557B0] border-[#E5E7EB] rounded-sm focus:ring-2 focus:ring-[#1557B0]/20 accent-[#1557B0] cursor-pointer"
                />
                <label htmlFor="isActive" className="text-sm font-medium text-[#1F2937] cursor-pointer select-none">
                  Display on Homepage Marquee (Active)
                </label>
              </div>

              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-[#1F2937]">Client Logo</label>
                <input
                  type="file"
                ...
                />
                <p className="text-xs text-[#6B7280]">Recommended: Landscape format (e.g. 300x100px) with transparent background. Max 5MB.</p>
              </div>

              {imagePreview && (
                <div className="h-28 rounded-sm overflow-hidden border border-[#E5E7EB] bg-[#F4F8FC] flex items-center justify-center p-4">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-sm shadow-2xl p-6 max-w-sm w-full">
            <h3 className="font-bold text-[#1F2937] mb-2">Delete Client?</h3>
